<?php
	...
	function convert_scenes($today)
	{
		...
		foreach ($today as $scene)
		
				{
					...
					$format_scenes[] = ['id'=>$scene['scene_id'],'date'=>$starting_time,'owner'=>$scene['owner_name'],'title'=>$scene['scene_title']];
				}
	...
	function draw_calendar($month,$year)
	{
	  ...
	  for ($x = 1; $x < 36; $x++):
		...
		{
			$today = $scenedb->select('volv_scene', ['owner_name', 'scene_date_scheduled', 'scene_title', 'scene_id'], ['scene_date_scheduled[<>]'=>[date("Y-m-d",mktime(0,0,0,$month,$day_counter,$year)),date("Y-m-d",mktime(0,0,0,$month,$day_counter+1,$year))], 'ORDER'=>['scene_date_scheduled']]);
			if($today)
			{
