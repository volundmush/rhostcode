<?php
/**
 * MUSH extension - A parser-function for converting RhostMUSH ANSI color markup to CSS.
 *
 ...
 * @file
 * @ingroup Extensions
 * @author Andrew "Volund" Bastien [https://github.com/volundmush/rhostcode ]
 */
...
function isStateReset($state) {
    return !isset($state['bold']) && !isset($state['underscore']) &&
           !isset($state['blink']) && $state['foreground'] == 7 &&
           $state['background'] == 0;
}

function htmlForState($state) {
    $ret = '<span style="';
    if (isset($state['bold']) && $state['bold']) {
        $ret .= "font-weight:bold;";
    }
    if (isset($state['underscore']) && $state['underscore']) {
        $ret .= "text-decoration:underline;";
    }
    if (isset($state['blink']) && $state['blink']) {
        $ret .= "text-decoration:blink;";
    }

    $fg = colorIndexToHtml(isset($state['bold']) && $state['bold'], $state['foreground']);
    $bg = colorIndexToHtml(false, $state['background']);

    if (isset($state['reverse']) && $state['reverse']) {
        $ret .= "color:" . $bg . ";";
    ...
    while ($offset < $len) {
        ...
        if ($byte === "\e") {
            if ($offset + 1 < $len && $text[$offset + 1] === "[") {
                ...
                while ($offset < $len && preg_match('/[0-9;]/', $text[$offset])) {
                    $command .= $text[$offset++];
                }

                $fromReset = isStateReset($state);
                $state = updateState($state, $command);
                $toReset = isStateReset($state);
...
class MUSH {
	...
	public static function MUSHParse( &$parser, $line)  {
        ...
        if ( empty( $wgRhost ) ) {
            return '<div class="error">RhostMUSH binary path not configured.</div>';
        }

        // Sanitize the input
        $sanitizedInput = escapeshellarg( $line );

        // Construct the command
        $command = sprintf('%s -p %s', escapeshellcmd($wgRhost), $sanitizedInput);

        // Execute the binary with the input
        $output = shell_exec( $command );

        if ( $output === null ) {
            return '<div class="error">Error executing RhostMUSH binary.</div>';
        }

        // run the output through ansiToSpans...
        $ansioutput = ansiToSpans($output);
